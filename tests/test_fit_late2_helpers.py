...
def test_chi_peaks_returns_arrays_and_peaks():
    p = TBParams()
    peaks_q, q_grid, chi_axial, chi_diag = chi_peaks_along_a(p, T=0.026, nk_bz=60, nk_q=25)
    assert q_grid.shape == chi_axial.shape == chi_diag.shape == (25,)
    assert isinstance(peaks_q, list)


def test_objective_finite_at_default():
    x = np.array([2.0, 0.37, 0.16, 0.0, -1.53])   # add t_2=0.0 as 4th element
...
def test_objective_returns_large_for_bad_params():
    ...
    x = np.array([2.0, 0.37, 0.16, 0.0, -10.0])   # add t_2=0.0 as 4th element
    val = objective(x, w_FS=1.0, w_q=1.0, nk_bz=60, nk_q=25)
    assert val >= 1e5
